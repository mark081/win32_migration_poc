...
- [x] 4.2 Implement the client capability cache and endpoint router
  ...
  - Completion evidence (2026-09-03): added a dependency-free process-local capability cache and endpoint router that bootstraps through the configured HTTPS Connected endpoint, sends the bounded client version, accepts only current schema 1 parent-enabled `compare`/`service` responses with bounded configuration versions and lifetimes, and selects Legacy for absent, disabled, Legacy, malformed, duplicate-field, unsupported, future-dated, expired, network, authentication, or unconfigured states. Product routing now refreshes only after cache expiry and pairs the selected endpoint with its own external credential; capability data remains routing metadata and cannot authorize business work. The Release build passed with the existing 6 warnings and 0 errors. The focused native harness and full non-UI gate passed after resetting contaminated synthetic demo data and starting the freshly built API; native/router, 32 service feature/telemetry/rule/API checks, database tests, and API integration tests all passed. Clang-format 19 dry-run and `git diff --check` passed. A later run from active unlocked RDP session 2 passed all 7 FlaUI tests with 0 failures and 0 skips. `graphify update .` rebuilt 826 nodes/1,556 edges; affected runtime, system, API, operations, and risk concepts were revised while gating, ownership, constraints, glossary, and open questions were verified unchanged; manifest `019debc291402cec69724d410e6f848cb52d07394f8bf82c5a4cb3432c2fa2e4` was written and strict OKF v0.2 validation passed.

### 5. Checkout mode integration

- [x] 5.1 Integrate Legacy and compare checkout client flows
  ...
  - Completion evidence (2026-09-03): added a structured NativeRules adapter and compare-only request serializer, then integrated compare routing without changing the existing confirmation or single idempotent checkout command. Valid compare responses must preserve the native allow/reason; malformed, stale, mismatched, or failed comparisons invalidate cached capability evidence, warn safely, and retain the Legacy result. PostgreSQL remains the only checkout writer. Release build passed with the existing 6 warnings and 0 errors. The focused native harness passed every stable reason mapping and proved the comparison carries one observation with no tool ID, command, or idempotency key. After resetting synthetic demo data and starting the freshly built API, the full non-UI gate passed native/router, all 32 service feature/telemetry/rule/capability/decision checks, database tests, and API integration tests, including compare match/mismatch/read-failure evidence and unchanged workflow/idempotency/audit state. From active unlocked RDP session 2, all 8 FlaUI tests passed with 0 failures and 0 skips; the added test confirms checkout cancellation produces no request or success output. Clang-format 19.1.5 dry-run, CSharpier 1.3.0, and `git diff --check` passed. `graphify update .` rebuilt 845 nodes/1,608 edges; affected runtime, system, API, operations, and risk concepts were revised while gating, ownership, constraints, glossary, and open questions were verified unchanged; manifest `5c9a61f0a37d9c8b8be61b6ae0208c38d7bb4bfc132744a2d45b8f448229e28e` was written and strict OKF v0.2 validation passed.

- [x] 5.2 Integrate the service-mode checkout client flow
  ...
  - Completion evidence (2026-09-04): service mode now branches before the member-detail read and NativeRules call, sends only syntactically validated member/date input plus the cached configuration version to the read-only decision route, accepts only a current version 1 service response with a known consistent reason, and maps stable denials to operator-safe text. Transport, stale, malformed, and contradictory responses stop without success; an allow still requires confirmation before the unchanged single idempotent PostgreSQL-backed command. SQLSTATE `40001` maps to stable `409 CONCURRENT_UPDATE` after PostgreSQL rolls back the attempt. The Release build passed with 0 errors and the existing warnings; the full non-UI gate passed native/transport, 33 feature, database, and API integration tests. After reseeding synthetic demo data and aligning the temporary UI credential with the synthetic local API key, an active unlocked RDP run passed all 9 FlaUI tests with 0 failures and 0 skips. Focused native and feature executables and `git diff --check` passed again during reconciliation. `graphify update .` rebuilt 860 nodes/1,676 edges; runtime, system, API, data, operations, and risk concepts were revised while gating, constraints, glossary, and open questions were verified unchanged. Manifest `b906083c8b29b6cfcf3a1a53d5c57116a64f5e8dfc33b5eb2577817b28777e38` was written and strict OKF v0.2 validation passed.

### 6. Cross-boundary verification and operations

- [x] 6.1 Expand service, API, and database migration-mode integration coverage
  ...
  - Completion evidence (2026-09-04): added a deterministic disabled/Legacy/compare/service matrix to the focused service executable using one real synthetic PostgreSQL fixture. Every row verifies expected effective mode, parent/child telemetry count, compare-record count, and identical workflow/idempotency/business-audit fingerprints; disabled and Legacy rows reject decision routing, while compare and service return the expected allow without a write. Existing evaluator cases retain parent-off/child-on, invalid, expired, target-miss, source-failure, stale capability, mismatch, telemetry failure, and read-failure coverage; API integration retains command idempotency, audit, and concurrent single-winner evidence. The Release build passed with the existing 6 warnings and 0 errors, and the full non-UI gate passed native/transport, 34 feature, database, and API integration checks.

- [x] 6.2 Expand Win32/FlaUI behavior and failure coverage
  ...
  - Completion evidence (2026-09-04): expanded the dependency-free client harness to verify distinct operator-safe text for configuration, timeout, unavailable-service, authentication, authorization, validation, conflict, and unexpected failures while rejecting any failure body as success. Existing service-mode checks prove no NativeRules call or member-policy input, cover every stable denial reason, reject malformed/stale/contradictory responses, and preserve no-success-before-commit behavior. FlaUI diagnostics now name the expected dialog. After a clean Release build, full non-UI pass, synthetic-data reset, and freshly built local API start, an active unlocked RDP run passed all 9 FlaUI tests in 2 seconds with 0 failures and 0 skips, covering accessibility IDs, administration validation, credential-value redaction, checkout validation, and cancellation without success output.

- [x] 6.3 Document configuration, deployment, observability, rollback, and flag lifecycle
  ...
  - Completion evidence (2026-09-04): added `docs/connected-operations.md` and linked it from README/testing guidance. It records the Connected objective and non-goals; retained NativeRules/PostgreSQL ownership; server snapshot schema, cache, atomic replacement, safe defaults, targeting, and disabled synthetic example; desktop HTTPS/credential/timeout settings; telemetry fields and redaction; mode/version compatibility; comparison observation boundary; default 60-second rollback convergence target; same-key ambiguous-write recovery; child-flag/NativeRules retirement condition; operational burden; and exact local/Connected verification boundaries. Internal target files, `git diff --check`, CSharpier, secret-pattern scan, and strict OKF v0.2 validation passed. `graphify update .` rebuilt 871 nodes/1,703 edges; operations knowledge was revised while architecture, interfaces, ownership, constraints, risks, glossary, and open questions were verified unchanged; manifest `158d4ba6c2507188982256829a775edf51f7b072c2d5c8d1f8c78009445477fe` was written.
